...
configSet = config.supportSendSMSThirty;
var sendInfo = {
    numberArr: [],
    total: 0,
    currentNum: "",
...
function ucs2_number_check(str) {
    ...
    for (i = 0; i < str.length; i++) {
        ...
        num_char_i = char_i.charCodeAt();
        for (j = 0; j < GSM_7BIT_NUM; j++) {
            if ((num_char_i == arrayGSM_7DefaultTable[j]) || (num_char_i == arrayGSM_7ExtTable[j]) || (config.isLatamSMS && language=="sp_latam" && (num_char_i == arrayLatamTable[j]))) {
                flag = 1;
                break;
            }
        }
...
function startQueueEvent(type, currContactId, draftId, smsCont, refurbishBack) {
    ...
    if (inputNumbers != "") {
        ...
        if (type == "send") {
            if (messageContent == "") {
                sys.confirm("ids_sms_emptyMessageWarn", function() {
                    sendEvent("");
                });
            } else {
                sendEvent("");
...
function sendEvent(number, smsId, smsCont, refurbishBack) {
    ...
    if (refurbishBack && refurbishBack != null) {
        ...
        result = SDK.SMS.SendSMS(SMS_SEND_SAVE_NEW, (config.isLatamSMS&&language=="sp_latam") ? to7BITStr(smsContent) : smsContent, arrayPhoneNum, smsTime);
    }
...
function saveEvent(number) {
    ...
    if (arrayPhoneNum.length > 0) {
        //page.startLoading();
        var result = SDK.SMS.SaveSMS(SMS_SEND_SAVE_NEW, (config.isLatamSMS&&language=="sp_latam") ? to7BITStr(smsContent) : smsContent, arrayPhoneNum, smsTime);
        if (result == API_RESULT_SUCCESS) {
...
function to7BITStr(str) {
    ...
    for (i = 0; i < str.length; i++) {
        ...
        char_i = str.charAt(i);
        num_char_i = char_i.charCodeAt();
        var positionNnm = position(num_char_i);
...
function checkCount(isFirst) {
    ...
    if (leftCount - (G_numCounts*addCount) >= 0) {
        return true
    } else {
        return false;
    }
...
function getSmsCountStr(strLen, type) {
    ...
    if (strLen <= singleStrNumA) {
        ...
        if(configSet.isActive && configSet.isActive != undefined){
            var phoneNum = $("#chosenUserSelect_chzn").children(".chzn-choices").children(".search-choice").size();
            if(phoneNum*counts > 30){
                $("#smsCharCount").addClass("colorRed");
                return;
            }else{
                $("#smsCharCount").removeClass("colorRed");
            }
        }
...
function _initContactSelect(config) {
    ...
    var phoneNumberCount = 3;
    configSet = config;
    if(config.isActive && config.isActive != undefined){
        phoneNumberCount = config.phoneNumberCount;
    }
    select.append(opts);
    select.chosen({
        max_selected_options: phoneNumberCount,
